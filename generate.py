...
Path("src/one_record_ontology/namespaces").mkdir(parents=True, exist_ok=True)


__INIT__ = """\
from one_record_ontology.namespaces._API import API
from one_record_ontology.namespaces._CARGO import CARGO
from one_record_ontology.namespaces._CODELISTS import CODELISTS
...
"""
with open("src/one_record_ontology/namespaces/__init__.py", "w") as f:
    f.write(__INIT__)
...
Path("_API.py").replace("src/one_record_ontology/namespaces/_API.py")
...
Path("_CARGO.py").replace("src/one_record_ontology/namespaces/_CARGO.py")
...
Path("src/one_record_ontology/namespaces/code_lists").mkdir(parents=True, exist_ok=True)
Path("src/one_record_ontology/namespaces/code_lists/__init__.py").touch()
...
for index, row in df.iterrows():
    ...
    if re.match(r"https://onerecord.iata.org/ns/code-lists/\w+#", target_namespace):
        ...
        Path(f"_{object_id}.py").replace(
            f"src/one_record_ontology/namespaces/code_lists/_{object_id}.py"
        )


_CODELISTS = f'''\
from rdflib.namespace import DefinedNamespace, Namespace
{"\n".join([f"from one_record_ontology.namespaces.code_lists._{object_id} import {object_id}" for _, object_id in object_ids])}
...
'''
with open("src/one_record_ontology/namespaces/_CODELISTS.py", "w") as f:
    f.write(_CODELISTS)


subprocess.run(
    ["uvx", "ruff", "check", "--fix", "src/one_record_ontology/namespaces"],
    check=True,
)
subprocess.run(
    ["uvx", "ruff", "format", "src/one_record_ontology/namespaces"],
    check=True,
)
